...
class editUserStory(unittest.TestCase):

    __PROJECT_NAME = "projet_editus_test"
    __TEST_PROJECT_ENTRY = "//a[@id='backlog-projet_editus_test']"
    _US_DESC_EDITED = "new_desc"
    ...
    def test_us(self, ):
        ...
        editBtn = "//td[@id='edit_btn']"
        editBtn = browser.find_element_by_xpath(editBtn)
        editBtn.click()
        waitURL(self.__chrome, File.EDITUS)

        usdesc = browser.find_element_by_xpath(Xpath.US_DESC_FIELD)
        usdesc.send_keys(_US_DESC_EDITED)

        self.__chrome.find_element_by_xpath(Xpath.VALID_EDITUS_BTN).click()
        waitURL(self.__chrome, File.BACKLOG)
